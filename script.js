...
class CommunicationApp {
                ...
                <form id="createCategoryForm">
                    ...
                    <div style="margin-bottom: 20px;">
                        <label style="display: block; font-size: 16px; font-weight: 600; color: #333; margin-bottom: 8px;">
                            <i class="fas fa-image" style="margin-right: 6px;"></i> Escolha uma imagem:
                        </label>
                        <div style="display: flex; gap: 10px; margin-bottom: 12px; flex-wrap: wrap;">
                            <button type="button" id="btnBuscarARASAAC" style="flex: 1; min-width: 150px; padding: 12px; background: #9C27B0; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                <i class="fas fa-search"></i> Buscar Pictograma (ARASAAC)
                            </button>
                            <button type="button" id="selectCategoryImageBtn" style="flex: 1; min-width: 150px; padding: 12px; background: #2196F3; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                <i class="fas fa-folder-open"></i> Galeria Local
                            </button>
                        </div>
                        ...
                        <div id="selectedCategoryImagePreview" style="margin-top: 12px; display: none;">
                            ...
                            <p style="margin-top: 8px; font-size: 14px; color: #666; text-align: center;">Imagem selecionada</p>
    ...
    async buscarPictogramasARASAAC(termo) {
        if (!termo || !termo.trim()) {
            return { success: false, error: 'Por favor, digite uma palavra para buscar.' };
        }

        try {
            const apiUrl = `https://api.arasaac.org/api/pictograms/pt/search/${encodeURIComponent(termo.trim())}`;
            ...
            if (!Array.isArray(dados) || dados.length === 0) {
                return { success: false, error: 'Nenhum pictograma encontrado.' };
            }

            return { success: true, pictogramas: dados.slice(0, 20) };
    ...
    openARASAACSearchModal(onSelect) {
        ...
        searchModal.innerHTML = `
            <div class="modal-content" role="dialog" aria-modal="true" style="max-width: 850px; max-height: 90vh; overflow-y: auto;">
                <h2 style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px; color: #333;">
                    <i class="fas fa-search" style="color: #007bff;"></i> Buscar Pictograma - ARASAAC
                </h2>
                
                <div style="padding: 12px; background: #d1ecf1; border-left: 4px solid #0c5460; margin-bottom: 20px; border-radius: 4px; font-size: 13px; color: #0c5460;">
                    <i class="fas fa-info-circle"></i> Digite uma palavra em português para buscar pictogramas (ex: comer, dormir, escola, casa)
                </div>
                
                <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                    <input type="text" id="inputBuscaPictograma" placeholder="Digite uma palavra (ex: comer, escola, brincar)" 
                ...
                <div id="resultadosPictogramas" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 15px; margin-bottom: 20px; min-height: 200px; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #f9f9f9;">
                    <div style="text-align: center; padding: 40px; color: #666; grid-column: 1 / -1;">
                        ...
                        <p>Digite uma palavra e clique em "Buscar" para ver os pictogramas disponíveis.</p>
                    </div>
                </div>
        ...
        const performSearch = async () => {
            ...
            if (!termo) {
                this.showError('Por favor, digite uma palavra para buscar');
                return;
            }
            ...
            resultadosDiv.innerHTML = `
                <div style="text-align: center; padding: 40px; color: #666; grid-column: 1 / -1;">
                    ...
                    <p style="margin-top: 10px;">Buscando pictogramas...</p>
                </div>
            `;
            ...
            if (!resultado.pictogramas || resultado.pictogramas.length === 0) {
                resultadosDiv.innerHTML = `
                    <div style="text-align: center; padding: 40px; color: #666; grid-column: 1 / -1;">
                        Nenhum pictograma encontrado para "${termo}"
                    </div>
                `;
                return;
            }
            ...
            <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="speechModalTitle">
                ...
                <form id="createSpeechForm">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <p style="margin-top: 8px; font-size: 13px; color: #999; font-style: italic;">
                            <i class="fas fa-info-circle"></i> Se não escolher imagem, será gerada automaticamente
                        </p>
                    </div>
