...
const translations = {
    'pt-BR': {
        ...
        'deleteAllCategories': 'Excluir Todas as Categorias',
        'close': 'Fechar',
        'save': 'Salvar',
    ...
    'es-ES': {
        ...
        'deleteAllCategories': 'Eliminar Todas las Categorías',
        'close': 'Cerrar',
        'save': 'Guardar',
...
class CommunicationApp {
    ...
    openNextSubcategoryForEdit() {
        ...
        // Buscar o item completo no localStorage
        const customSpeeches = JSON.parse(localStorage.getItem('customSpeeches') || '{}');
        const categorySpeeches = customSpeeches[this.currentCategory] || [];
    ...
    showSubCategories(categoryId) {
        try {
            ...
            const customSpeeches = JSON.parse(localStorage.getItem('customSpeeches') || '{}');
            const categoryCustomSpeeches = customSpeeches[categoryId] && Array.isArray(customSpeeches[categoryId])
    ...
    renderSubcategoryBatch() {
        ...
        for (let i = this.subcategoryRenderOffset; i < end; i++) {
            const entry = queue[i];
            if (!entry) continue;
            const card = this.createSubCategoryCard(entry.item, entry.itemIndex);
    ...
    openSettingsModal() {
                ...
                <div class="settings-content" style="flex: 1; overflow-y: auto;">
                    ...
                    <div style="margin-top: 25px; padding-top: 25px; border-top: 2px solid #e9ecef;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            ...
                            <button type="button" id="deleteAllCategoriesBtn" style="width: 100%; padding: 12px; background: #e74c3c; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 10px;">
                                <i class="fas fa-trash-alt"></i> ${t('deleteAllCategories')}
                            </button>
                        </div>
                    </div>
        ...
        const deleteAllBtn = document.getElementById('deleteAllCategoriesBtn');
        if (deleteAllBtn) {
    ...
    renderStatsPanelContent(period) {
    ...
    updateSettingsModalLanguage(lang) {
        ...
        if (deleteAllCategoriesBtn) {
            const icon = deleteAllCategoriesBtn.querySelector('i');
            deleteAllCategoriesBtn.innerHTML = icon ? `${icon.outerHTML} ${t('deleteAllCategories')}` : t('deleteAllCategories');
        }
    ...
    deleteAllCategories() {
        try {
            ...
            if (customCategories.length === 0) {
                this.showError('Não há categorias para excluir');
                return;
            }
            ...
            const confirmDelete = confirm(
                `Tem certeza que deseja excluir TODAS as ${customCategories.length} categorias?\n\n` +
            ...
            // Pega todos os IDs
            const allCategoryIds = customCategories.map(cat => cat.id);
            
            // Exclui todas
            this.deleteCategoriesMultiple(allCategoryIds);
            
            // Volta para tela principal se estava dentro de uma categoria
            if (this.currentCategory) {
                this.goBack();
            }
        } catch (error) {
            this.showError('Erro ao excluir todas as categorias: ' + error.message);
