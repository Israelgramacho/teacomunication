...
class GoogleDriveManager {
    ...
    // Verifica se as APIs do Google estão disponíveis
    checkGoogleAPIs() {
        const checks = {
            gapi: !!window.gapi,
            gapiLoad: !!(window.gapi && window.gapi.load),
        ...
        console.log('Verificação de APIs do Google:', checks);
        return checks;
    }
    ...
    async init() {
        ...
        try {
            ...
            const checks = this.checkGoogleAPIs();
            
            if (!checks.gapi) {
                throw new Error('Google API (gapi) não está carregada. Verifique se o script https://apis.google.com/js/api.js foi carregado.');
            }
                ...
                if (window.gapi && window.gapi.load) {
                    ...
                    const checkInterval = setInterval(() => {
                        ...
                        if (window.gapi && window.gapi.load) {
                            ...
                            clearInterval(checkInterval);
                            reject(new Error('Google API (gapi.load) não carregou após 5 segundos. Verifique sua conexão com a internet.'));
                        }
                    }, 100);
                }
            });

            // Carrega o Client ID do localStorage
            this.clientId = localStorage.getItem('googleDriveClientId');
            
            if (!this.clientId) {
                const error = 'Google Drive Client ID não configurado. Por favor, configure o Client ID nas configurações da aplicação.';
                console.warn(error);
            ...
            await window.gapi.client.init({
                apiKey: null, // Não precisa de API key para OAuth
    ...
    // Configura o Client ID
    setClientId(clientId) {
        this.clientId = clientId;
        localStorage.setItem('googleDriveClientId', clientId);
        this.isInitialized = false; // Força reinicialização
    }
    ...
    async authenticate() {
        try {
            if (!this.isInitialized) {
                ...
                if (!initialized) {
                    throw new Error('Google Drive não foi inicializado. Configure o Client ID nas configurações.');
                }
            }
            ...
            if (this.accessToken) {
                ...
                try {
                    ...
                    // Token inválido, precisa reautenticar
                    this.accessToken = null;
                }
            }
            ...
            await new Promise((resolve) => {
                if (window.google && window.google.accounts && window.google.accounts.oauth2) {
                    ...
                    setTimeout(() => {
                        clearInterval(checkInterval);
                        resolve(); // Continua mesmo se não carregar
                    }, 5000);
                }
            });

            // Usa Google Identity Services (GSI) para autenticação
            return new Promise((resolve, reject) => {
                if (!window.google || !window.google.accounts || !window.google.accounts.oauth2) {
                    reject(new Error('Google Identity Services não está disponível. Verifique se o script foi carregado corretamente.'));
                    return;
                }
    ...
    async saveFile(filename, content, mimeType = 'application/json') {
        try {
            ...
            // Cria o arquivo usando multipart upload
            ...
            if (!response.ok) {
                ...
                throw new Error(error.error?.message || 'Erro ao salvar no Google Drive');
            }
    ...
    async listFiles(query = '') {
        try {
            ...
            url += encodeURIComponent(`mimeType='application/json' and name contains 'comunicacao-alternativa'`);
            url += '&fields=files(id,name,createdTime,modifiedTime)';
    ...
    async downloadFile(fileId) {
        try {
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.error?.message || 'Erro ao baixar arquivo');
            }
...
// Instância global do GoogleDriveManager
...
class CommunicationApp {
    ...
    openSettingsModal() {
                        ...
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <button type="button" id="exportDataBtn" style="padding: 12px; background: #28a745; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                <i class="fas fa-download"></i> ${t('export')}
                                    ...
                                    <button type="button" id="exportToDriveBtn" style="padding: 12px; background: #4285f4; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                        <i class="fab fa-google-drive"></i> ${t('exportToDrive') || 'Salvar no Google Drive'}
                                    </button>
                            <label style="display: block;">
                                ...
                                <button type="button" id="importDataBtn" style="width: 100%; padding: 12px; background: #007bff; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                    ...
                                    <button type="button" id="importFromDriveBtn" style="width: 100%; padding: 12px; background: #34a853; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                        <i class="fab fa-google-drive"></i> ${t('importFromDrive') || 'Importar do Google Drive'}
                                    </button>
                                </div>
                            </div>
                            <div style="margin-top: 25px; padding-top: 25px; border-top: 2px solid #e9ecef;">
                                <h3 style="font-size: 16px; margin-bottom: 15px; color: #333;"><i class="fab fa-google-drive"></i> ${t('googleDriveConfig') || 'Configuração Google Drive'}</h3>
                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                    <label style="font-size: 14px; color: #666;">
                                        ${t('googleDriveClientId') || 'Client ID do Google Drive:'}
                                        <input type="text" id="googleDriveClientIdInput" 
                                            placeholder="${t('googleDriveClientIdPlaceholder') || 'Cole seu Client ID aqui'}" 
                                            style="width: 100%; padding: 8px; margin-top: 5px; border: 2px solid #ddd; border-radius: 6px; font-size: 14px; box-sizing: border-box;"
                                            value="${localStorage.getItem('googleDriveClientId') || ''}">
                                    </label>
                                    <small style="color: #666; font-size: 12px;">
                                        ${t('googleDriveClientIdHelp') || 'Para obter um Client ID, acesse: https://console.cloud.google.com/apis/credentials'}
                                    </small>
                                    <button type="button" id="saveGoogleDriveConfigBtn" style="padding: 10px; background: #4285f4; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; margin-bottom: 10px;">
                                        ${t('save') || 'Salvar Configuração'}
                                    </button>
                                    <button type="button" id="testGoogleDriveBtn" style="padding: 10px; background: #34a853; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600;">
                                        <i class="fas fa-vial"></i> ${t('testConnection') || 'Testar Conexão'}
                                    </button>
                                </div>
                            </div>
        ...
        document.getElementById('exportToDriveBtn').onclick = () => this.exportToDrive();
        ...
        document.getElementById('importFromDriveBtn').onclick = () => this.importFromDrive();
        document.getElementById('saveGoogleDriveConfigBtn').onclick = () => {
            const clientId = document.getElementById('googleDriveClientIdInput').value.trim();
            if (clientId) {
                googleDriveManager.setClientId(clientId);
                this.updateStatus('Configuração do Google Drive salva!');
            } else {
                this.showError('Por favor, insira um Client ID válido.');
            }
        };
        
        document.getElementById('testGoogleDriveBtn').onclick = async () => {
    ...
    async importData(event) {
        ...
        try {
            ...
            // NOVO: Importar Estatísticas se existirem no arquivo
            if (importData.statistics) {
                 localStorage.setItem('usageStats', JSON.stringify(importData.statistics));
            } else if (importData.stats) {
                 // Compatibilidade com formatos anteriores se houver
                 localStorage.setItem('usageStats', JSON.stringify(importData.stats));
            }
    ...
    async exportToDrive() {
        try {
            this.showLoading('Preparando exportação para Google Drive...');
            
            // Verifica se Google Drive está disponível
            const checks = googleDriveManager.checkGoogleAPIs();
            if (!checks.gapi || !checks.google) {
                this.hideLoading();
                this.showError('Google Drive não está disponível. Verifique se:\n\n1. Você tem conexão com a internet\n2. As APIs do Google foram carregadas corretamente\n3. O Client ID está configurado nas configurações');
                return;
            }

            // Verifica se Client ID está configurado
            const clientId = localStorage.getItem('googleDriveClientId');
            if (!clientId) {
                this.hideLoading();
                this.showError('Google Drive não está configurado. Por favor:\n\n1. Vá em Configurações\n2. Role até "Configuração Google Drive"\n3. Cole seu Client ID do Google Cloud Console\n4. Clique em "Salvar Configuração"');
                return;
            }
            ...
            this.hideLoading();
            this.showLoading('Autenticando no Google Drive...');
            
            // Salva no Google Drive
            const file = await googleDriveManager.saveFile(filename, exportData);
            
            this.hideLoading();
            const lang = localStorage.getItem('appLanguage') || 'pt-BR';
            const t = (key) => getTranslation(key, lang);
            this.updateStatus(t('dataExported') + ' no Google Drive!');
        } catch (error) {
            this.hideLoading();
            let errorMessage = 'Erro ao exportar para Google Drive: ' + error.message;
            
            // Mensagens mais específicas
            if (error.message.includes('Client ID')) {
                errorMessage = 'Google Drive não está configurado. Configure o Client ID nas configurações.';
            } else if (error.message.includes('não carregou') || error.message.includes('não está disponível')) {
                errorMessage = 'Google Drive não está disponível. Verifique sua conexão com a internet e se as APIs do Google foram carregadas.';
            } else if (error.message.includes('autenticação') || error.message.includes('authentication')) {
                errorMessage = 'Erro na autenticação do Google. Tente novamente.';
            }
            ...
            console.error('Detalhes:', {
                ...
                stack: error.stack,
                checks: googleDriveManager.checkGoogleAPIs()
            });
        }
    }
    ...
    async importFromDrive() {
        try {
            ...
            this.showLoading('Carregando arquivos do Google Drive...');
            ...
            if (files.length === 0) {
                this.showError('Nenhum arquivo de backup encontrado no Google Drive.');
                return;
            }

            // Mostra modal para selecionar arquivo
            const lang = localStorage.getItem('appLanguage') || 'pt-BR';
            const t = (key) => getTranslation(key, lang);
            ...
            return `
                    <div style="padding: 12px; border: 2px solid #ddd; border-radius: 6px; margin-bottom: 10px; cursor: pointer; transition: all 0.3s;"
                         onmouseover="this.style.borderColor='#4285f4'; this.style.background='#f0f7ff';"
                        ...
                        <div style="font-size: 12px; color: #666;">Modificado: ${dateStr}</div>
                </div>
            `;
        }).join('');
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 600px;">
                    ...
                    <div style="display: flex; gap: 10px; justify-content: flex-end;">
                        <button id="cancelImportDriveBtn" style="padding: 10px 20px; background: #6c757d; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 16px;">
                            ${t('cancel') || 'Cancelar'}
                        </button>
                    </div>
                </div>
            ...
            // Adiciona event listeners para selecionar arquivo
            modal.querySelectorAll('[data-file-id]').forEach(item => {
                item.addEventListener('click', async () => {
                    const fileId = item.dataset.fileId;
                    modal.remove();
                    
                    try {
                        this.showLoading('Importando do Google Drive...');
                        
                        // Baixa o arquivo
                        const fileContent = await googleDriveManager.downloadFile(fileId);
                        ...
                        this.hideLoading();
                        this.showError('Erro ao importar do Google Drive: ' + error.message);
                        console.error('Erro ao importar do Google Drive:', error);
                    }
                });
            });
            
            document.getElementById('cancelImportDriveBtn').onclick = () => modal.remove();
            modal.onclick = (e) => {
                if (e.target === modal) modal.remove();
            };
        } catch (error) {
            this.hideLoading();
            let errorMessage = 'Erro ao acessar Google Drive: ' + error.message;
    ...
    async testGoogleDriveConnection() {
        try {
            this.showLoading('Testando conexão com Google Drive...');
            
            // Verifica APIs
            const checks = googleDriveManager.checkGoogleAPIs();
            let report = '=== DIAGNÓSTICO GOOGLE DRIVE ===\n\n';
            
            report += 'APIs Carregadas:\n';
            report += `- Google API (gapi): ${checks.gapi ? '✅' : '❌'}\n`;
            report += `- gapi.load: ${checks.gapiLoad ? '✅' : '❌'}\n`;
            report += `- Google Identity: ${checks.google ? '✅' : '❌'}\n`;
            report += `- Google Accounts: ${checks.googleAccounts ? '✅' : '❌'}\n`;
            report += `- OAuth2: ${checks.googleOAuth2 ? '✅' : '❌'}\n\n`;
            
            // Verifica Client ID
            const clientId = localStorage.getItem('googleDriveClientId');
            report += `Client ID Configurado: ${clientId ? '✅ (' + clientId.substring(0, 20) + '...)' : '❌'}\n\n`;
            
            // Verifica conexão
            try {
                const response = await fetch('https://www.googleapis.com/discovery/v1/apis/drive/v3/rest', { method: 'HEAD' });
                report += `Conexão com Google APIs: ${response.ok ? '✅' : '❌ (' + response.status + ')'}\n\n`;
            } catch (e) {
                report += `Conexão com Google APIs: ❌ (${e.message})\n\n`;
            }
            
            // Tenta inicializar
            if (clientId && checks.gapi) {
                try {
                    await googleDriveManager.init();
                    report += 'Inicialização: ✅\n';
                } catch (e) {
                    report += `Inicialização: ❌ (${e.message})\n`;
                }
            } else {
                report += 'Inicialização: ⚠️ (Client ID ou APIs não disponíveis)\n';
            }
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 600px;">
                    <h2 style="margin-bottom: 15px;">
                        <i class="fas fa-vial"></i> Diagnóstico Google Drive
                    </h2>
                    <pre style="background: #f5f5f5; padding: 15px; border-radius: 6px; overflow-x: auto; white-space: pre-wrap; font-family: monospace; font-size: 12px;">${report}</pre>
                    <div style="margin-top: 15px; padding: 10px; background: #fff3cd; border-radius: 6px; font-size: 13px;">
                        <strong>Dicas:</strong><br>
                        ${!checks.gapi ? '• Recarregue a página para carregar as APIs do Google<br>' : ''}
                        ${!clientId ? '• Configure o Client ID nas configurações acima<br>' : ''}
                        • Verifique sua conexão com a internet<br>
                        • Se estiver em WebView, verifique se as URLs do Google estão permitidas
                    </div>
                    <button id="closeDiagnosticBtn" style="margin-top: 15px; padding: 10px 20px; background: #6c757d; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 16px; width: 100%;">
                        Fechar
                    </button>
                </div>
            `;
            document.body.appendChild(modal);
            document.getElementById('closeDiagnosticBtn').onclick = () => modal.remove();
            modal.onclick = (e) => {
                if (e.target === modal) modal.remove();
            };
            ...
            this.hideLoading();
            this.showError('Erro ao testar conexão: ' + error.message);
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Verifica APIs do Google após um delay (para dar tempo de carregar)
        setTimeout(() => {
            const checks = googleDriveManager.checkGoogleAPIs();
            if (!checks.gapi || !checks.google) {
                console.warn('APIs do Google não foram carregadas. Verifique se os scripts foram carregados corretamente.');
                console.warn('Verificação:', checks);
            }
        }, 2000);
